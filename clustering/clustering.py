...
def cmd_parser():
	...
	parser.add_argument('-v', '--w2v-input', type=str, help='W2V input path', required=True)
...
def mllib_kmeans(inputData, minClusters, maxClusters):
	...
	# --------------------------------------------------------------------------
	# inputData.cache()
	parsedData = inputData.map(lambda x: array([float(i) for i in x.split('\t')[1].split(',')]))
	parsedData.cache()
	#parsedData = data
	...
	#parsedData.repartition(100).saveAsTextFile('/tmp/avishek2/output/qlas_cat_w2v' + '_preds')
	preds.cache()
	
	#output = data.map(lambda x: (x.split('\t')[0],x.split('\t')[1])).join(preds)
	output = inputData\
		.map(lambda x: (x.split('\t')[1],x.split('\t')[0]))\
		.join(preds.map(lambda x: (x[0],x[1])))\
...
if __name__ == "__main__":
	...
	training_w_clusterid \
		.saveAsTextFile(output + '_training_w_clusterid')
	...
	testing = joined_testing\
		.map(lambda x: x.split('\t')[0] + '\t' + '-1' + '\t' + '-1') #\
		#.map(lambda x: x.encode('UTF8'))
	testing.repartition(100).saveAsTextFile(output + '_testing')
